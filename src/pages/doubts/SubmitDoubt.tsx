...
import { Send, BookOpen, User, MessageSquare, Menu, X, ShoppingCart, Award, LogOut } from 'lucide-react';
...
const SubmitDoubt: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex">
            ...
            <div className={`fixed lg:sticky top-0 inset-y-0 left-0 z-40 w-64 bg-white dark:bg-gray-900 h-screen border-r border-gray-200 dark:border-gray-800 flex flex-col transition-transform duration-300 ${showMobileSidebar ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
                }`}>
                ...
                <nav className="flex-1 overflow-y-auto px-6 pb-6 space-y-2">
                    {tabs.map((tab) => (
                        <Link
                            ...
                            to={tab.path}
                            onClick={() => setShowMobileSidebar(false)}
                            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${tab.id === 'doubts'
